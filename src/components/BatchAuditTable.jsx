...
export function BatchAuditTable({
  ...
  return <Table>
      <TableHeader>
        <TableRow>
          <TableHead className="w-12">
            <input type="checkbox" checked={columns.length > 0 && columns.every(column => selectedColumns.includes(column.id))} onChange={e => onSelectAll(e.target.checked)} className="rounded border-gray-300" />
          </TableHead>
      ...
      <TableBody>
        {columns.map(column => <React.Fragment key={column.id}>
              ...
              <TableCell>
                <div className="flex space-x-1">
                  <Button size="sm" variant="outline" onClick={() => onToggleExpand(column.id)} className="h-8 w-8 p-0" title="展开详情">
                    {expandedRows.includes(column.id) ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => onPreview(column.id)} className="h-8 w-8 p-0" title="预览详情">
